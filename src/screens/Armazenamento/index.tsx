/***
 * TELA DE CADASTRO DE TIPOS DE ARMAZENAMENTO
 * TODO:
 *
 */
...
const armColumns = [
  { name: "ID", key: "id" },
  { name: "TIPO ARMAZENAMENTO", key: "name" },
...
const ArmazenamentoScreen = ({
  ...
  const { data: mainDB, refetch } = useQuery(GET_ARMAZENAMENTO);

  const [upsert] = useMutation(UPSERT_ARMAZENAMENTO, {
    onCompleted: (data) => {
  ...
  useEffect(() => {
    if (
      mainDB &&
      mainDB.getArmazenamento &&
      mainDB.getArmazenamento.length > 0
    ) {
      setDbGrid([...mainDB.getArmazenamento]);
    }
  }, [mainDB]);
  ...
  const renderForm = () => (
    <>
      ...
      <div className="pt-5">
        <DynamicTable
          label="Tipos de Armazenamentos Cadastrados"
          data={dbGrid}
          columns={armColumns}
          hasDelete
          hasEdit
...
export default ArmazenamentoScreen;
